...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        subtitle = QLabel("Convierte personajes de renders 3D en personas fotorrealistas")
        ...
        self.api_key_input.setToolTip(
            "La clave se guarda en el almacén seguro de credenciales de tu sistema."
        )
        ...
        source_title_row.addWidget(self._section_label("Imágenes de entrada"))
        source_title_row.addStretch(1)
        self.selection_count = QLabel("0 imágenes")
        ...
        self.drop_hint = QLabel(
            "Arrastra aquí imágenes o carpetas. También puedes usar los botones inferiores."
        )
        ...
        self.add_files_button = QPushButton("Añadir imágenes")
        ...
        self.file_table.setHorizontalHeaderLabels(["ARCHIVO", "TAMAÑO", "ESTADO", "DETALLE"])
        ...
        source_layout.addWidget(self._section_label("Directorio de salida"))
        output_row = QHBoxLayout()
        self.output_edit = QLineEdit()
        self.output_edit.setPlaceholderText("Selecciona dónde guardar los resultados")
        self.output_edit.textChanged.connect(self._persist_output_directory)
        self.output_button = QPushButton("Seleccionar")
        self.open_output_button = QPushButton("Abrir")
        ...
        preview_header.addWidget(self._section_label("Previsualización"))
        preview_header.addStretch(1)
        self.preview_name = QLabel("Selecciona una imagen")
        ...
        self.original_preview = ImagePreview("Selecciona una imagen de la cola")
        self.result_preview = ImagePreview("El resultado aparecerá al completar la edición")
        self.preview_tabs.addTab(self.original_preview, "Original")
        self.preview_tabs.addTab(self.result_preview, "Resultado")
        ...
        status_layout.addWidget(self._section_label("Estado actual"))
        self.stage_label = QLabel("Listo para procesar")
        self.stage_label.setObjectName("StageLabel")
        self.stage_detail = QLabel("Añade uno o varios renders para comenzar.")
        ...
        self.log_toggle.setText("Ver registro")
    ...
    def _refresh_api_controls(self) -> None:
        ...
        self.api_key_input.setPlaceholderText(
            ...
            if ready
            else "Introduce tu clave de OpenAI (sk-…)"
        )
        self.api_key_button.setText(
            "API configurada · Guardar cambios" if ready else "Guardar API key"
        )
    ...
    def _store_api_key(self, *, show_confirmation: bool) -> bool:
        ...
        if not api_key:
            message = (
                "La clave ya está guardada. Introduce una nueva clave para sustituirla."
                if has_openai_api_key(self.credential_store)
                else "Introduce una clave de OpenAI."
            )
            QMessageBox.information(self, APP_NAME, message)
            return False
        if not api_key.startswith("sk-") or len(api_key) < 20:
            QMessageBox.warning(self, APP_NAME, "La clave de OpenAI no parece válida.")
            return False
        try:
        ...
        if show_confirmation:
            QMessageBox.information(
                self,
                APP_NAME,
                "Clave guardada localmente en el almacén seguro de tu sistema.",
            )
        return True
    ...
    def _toggle_log(self, visible: bool) -> None:
        self.log_toggle.setText("Ocultar registro" if visible else "Ver registro")
    ...
    def _choose_folder(self) -> None:
        start = str(self.settings.value("last_input_dir", str(project_root())))
        chosen = QFileDialog.getExistingDirectory(self, "Seleccionar carpeta de renders", start)
        if not chosen:
            return
        ...
        if not discovered:
            QMessageBox.information(self, APP_NAME, "No se encontraron imágenes compatibles en la carpeta.")
            return
        self._add_paths(discovered)

    def _choose_output(self) -> None:
        start = self.output_edit.text().strip() or str(project_root())
        chosen = QFileDialog.getExistingDirectory(self, "Seleccionar directorio de salida", start)
        if chosen:
    ...
    def _clear_files(self) -> None:
        ...
        self.preview_name.setText("Selecciona una imagen")
    ...
    def _rebuild_table(self) -> None:
        ...
        for row, path in enumerate(self.files):
            ...
            if result:
                ...
                status = "Completada" if result.status == "completed" else "Omitida"
                self._set_row_status(row, status, result.message, state)
        count = len(self.files)
        self.selection_count.setText(f"{count} imagen" if count == 1 else f"{count} imágenes")
    ...
    def _start_processing(self) -> None:
        ...
        if missing:
            QMessageBox.warning(self, APP_NAME, f"Ya no existe el archivo:\n{missing[0]}")
            return
        output_value = self.output_edit.text().strip()
        if not output_value:
            QMessageBox.warning(self, APP_NAME, "Selecciona un directorio de salida.")
            return
        try:
        ...
        existing_by_row = {
            ...
            if (paths := existing_result_paths(output_dir, source_path.stem))
        }
        skip_existing = False
        if existing_by_row:
            ...
            dialog.setText(
                f"Ya existen resultados para {len(existing_by_row)} de "
                f"{len(self.files)} imagen(es)."
            )
            dialog.setInformativeText(
                "¿Quieres volver a procesarlas? Los resultados anteriores se conservarán y "
                "los nuevos se guardarán con un sufijo como _2 o _3."
            )
            reprocess_button = dialog.addButton(
                "Reprocesar y renombrar",
                QMessageBox.AcceptRole,
            )
            skip_button = dialog.addButton(
                "Omitir las procesadas",
        ...
        for row, source_path in enumerate(self.files):
            if skip_existing and row in existing_by_row:
                ...
                message = f"Ya procesada: se conserva {existing_output.name}."
                ...
                self._set_row_status(row, "Ya procesada", message, "skipped")
            else:
                pending.append((row, source_path))
                self._set_row_status(row, "En cola", "", "pending")

        if not pending:
            self.total_progress.setRange(0, len(self.files))
            self.total_progress.setValue(len(self.files))
            self.stage_label.setText("No hay imágenes pendientes")
            self.stage_detail.setText(
                "Todos los resultados existentes se han conservado sin consumir la API."
            )
        ...
        if not has_openai_api_key(self.credential_store):
            QMessageBox.warning(
                self,
                APP_NAME,
                "Introduce y guarda una clave de OpenAI antes de procesar.",
            )
            self._rebuild_table()
            self._update_preview_from_selection()
            return
    ...
    def _on_item_stage(self, row: int, title: str, detail: str, step: int) -> None:
        filename = self.files[row].name if 0 <= row < len(self.files) else "Imagen"
        self.stage_label.setText(f"{title} · {filename}")
    ...
    def _on_item_finished(self, row: int, result: PipelineItemResult) -> None:
        ...
        if result.status == "completed":
            status, state = "Completada", "success"
        else:
            status, state = "Omitida", "skipped"
        self._set_row_status(row, status, result.message, state)
        if self._selected_row() == row:
            self._update_preview_from_selection()
    ...
    def _on_item_failed(self, row: int, message: str) -> None:
        ...
        self.stage_label.setText("Error de procesado")
    ...
    def _on_batch_finished(self, cancelled: bool, succeeded: int, failed: int, skipped: int) -> None:
        ...
        if cancelled:
            self.stage_label.setText("Procesado cancelado")
            self.stage_detail.setText(
                f"Completadas: {succeeded} · Sin procesar/omitidas: {skipped} · Errores: {failed}"
            )
        elif failed:
            self.stage_label.setText("Lote terminado con incidencias")
            self.stage_detail.setText(
                f"Completadas: {succeeded} · Omitidas/copias: {skipped} · Errores: {failed}"
            )
        else:
            self.stage_label.setText("Lote completado")
            self.stage_detail.setText(
                f"Completadas con IA: {succeeded} · Omitidas: {skipped}"
            )
    ...
    def _on_thread_stopped(self) -> None:
        ...
        self.worker = None
        self.thread = None
        self.process_button.setText("Procesar imágenes")
    ...
    def _cancel_processing(self) -> None:
        ...
        self.stage_detail.setText(
            "La cancelación se aplicará al terminar la operación activa. Una llamada a la API no puede interrumpirse a mitad."
        )
    ...
    def closeEvent(self, event) -> None:  # noqa: N802 - Qt API
        if self.processing:
            answer = QMessageBox.question(
                self,
                APP_NAME,
                "Hay un lote en proceso. ¿Quieres cancelarlo y cerrar cuando termine la operación activa?",
