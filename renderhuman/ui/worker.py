...
class ProcessingWorker(QObject):
    item_stage = pyqtSignal(int, str, str, int)
    ...
    def __init__(
        ...
        *,
        ...
        if len(self.row_indices) != len(self.files):
            raise ValueError("El número de filas no coincide con el número de imágenes.")
    ...
    def _process_item(
        ...
        self.log_message.emit(f"Iniciando: {source_path}")
    ...
    def _publish_outcome(
        ...
        if status == "failed":
            ...
            self.log_message.emit(f"Error en {source_path.name}: {message}")
            return 0, 1, 0, False

        if result is None:
            message = "El procesado no devolvió ningún resultado."
            self.item_failed.emit(row, message)
            self.log_message.emit(f"Error en {source_path.name}: {message}")
            return 0, 1, 0, False

        self.item_finished.emit(row, result)
        self.log_message.emit(f"{result.message} {source_path.name}")
    ...
    def run(self) -> None:
        ...
        try:
            ...
            message = f"{type(error).__name__}: {error}"
            self.log_message.emit(f"Error general: {message}")
            if self.row_indices:
